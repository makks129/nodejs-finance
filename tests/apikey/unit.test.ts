...
describe('apikey validation', () => {
  ...
  it('Should respond with 400 if x-api-key header is not passed', async () => {
    const response = await request.get(endpoint).timeout(2000);
    expect(response.status).toBe(400);
    expect(mockFindApiKey).not.toBeCalled();
  });

  it('Should respond with 403 if wrong x-api-key header is passed', async () => {
  ...
  it('Should respond with 404 if correct x-api-key header is passed and when route is not handelled', async () => {
